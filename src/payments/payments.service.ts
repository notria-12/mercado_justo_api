            ...
            if(user){
               ...
               if(responsePlan.data['Payment']['RecurrentPayment']['ReasonCode'] == 0){
                 ...
                 if(signature){
                     ...
                     await this.signatureModel.updateOne({id_usuario: createSignature.id_usuario}, {status:  true, data_expiracao:new Date(responsePlan.data['Payment']['RecurrentPayment']['NextRecurrency']).getTime(), ultima_assinatura:  Date.now(), id_assinatura: responsePlan.data['Payment']['RecurrentPayment']['RecurrentPaymentId'], tipo_pagamento: tipo[0], id_pagamento: responsePlan.data['Payment']['PaymentId'], pagamento_pendente: false});
                 }else{
                     const signature = new this.signatureModel({ status: true, data_expiracao:  new Date(responsePlan.data['Payment']['RecurrentPayment']['NextRecurrency']).getTime(), ultima_assinatura: Date.now(), id_usuario: createSignature.id_usuario, card_token: undefined, tipo_pagamento: tipo[0], id_pagamento: responsePlan.data['Payment']['PaymentId'], id_assinatura: responsePlan.data['Payment']['RecurrentPayment']['RecurrentPaymentId'], pagamento_pendente: false});
                     signature.save();
                 }
